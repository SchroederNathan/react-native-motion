export const meta = {
  ...
  slug: 'linear-tab-bar',
  description: 'A frosted-glass tab bar modeled on Linear\'s mobile app. Drag up and the pill morphs into a menu, drag past its edges and the glass stretches, and search grows out of it in place.',
...
A frosted-glass pill with tappable, swipeable tab icons, modeled on Linear's mobile app. Drag up to open a full menu with staggered rows, or tap the search control to grow it into a search bar while the pill collapses. Dragging past the pill's edges stretches the glass like liquid, and a radial glow tracks your finger.

The morph squeezes before it grows: scaleX dips around 15% progress, then the height expands into the menu. Overdrag stretches the glass along the drag axis with a damped curve and squeezes it slightly across it. The glow is a radial SVG gradient that sits under the finger and fades out on release. In search mode the pill's width goes to zero as the search bar grows and a close button slides in. The header, divider, and seven menu rows stagger in at 40ms intervals and reverse on close.
