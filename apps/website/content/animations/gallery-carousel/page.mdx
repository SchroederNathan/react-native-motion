export const meta = {
  ...
  description: 'Cards blend from a fanned stack into a centered carousel on the first swipe, while a blurred backdrop crossfades to match the active card.',
...
A stacked card carousel with a blurred backdrop that follows it. On index 0 the cards sit in a fanned deck; the first swipe blends them into a centered carousel. Behind the cards, the backdrop crossfades to a blurred copy of whichever image is active.
...
- `galleryFactor` only spans index 0 to 1. The stack-to-carousel blend is the first swipe; after index 1 you stay in full carousel. Don't widen the interpolation range.
