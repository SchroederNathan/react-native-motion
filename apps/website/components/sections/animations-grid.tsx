...
export function AnimationsGrid({ animations, animate = true }: { animations: AnimationMeta[]; animate?: boolean }) {
  return (
    <section className="py-16">
      <Container>
        <div className={`grid gap-6 ${animations.length === 1 ? 'mx-auto max-w-sm' : animations.length === 2 ? 'mx-auto grid-cols-1 sm:grid-cols-2 lg:max-w-2/3' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3'}`}>
