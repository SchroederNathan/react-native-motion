export const meta = {
  ...
  slug: 'aurora-curtain',
  description: 'A curtain of violet light drawn by a single Skia shader, looping without a seam. Drag down to pull more of it onto the screen.',
...
The lean is a time lag, not a drawn slant. Each ray reads its swing at a clock lagged by depth (`tw - ys * LAG`), so it leans against its own drift while moving and straightens as it settles. Ray centres are scattered off the even lattice and widths vary widely, because evenly spaced rays read as vertical stripes. Every rate in the shader is a whole-number multiple of a 14-second master clock driven by one linear, non-reversing `withRepeat`, which is what lets the loop wrap with no visible seam.

More of the curtain hides above the screen than shows. Dragging down is a plain ScrollView overscroll, so the hidden part follows the finger 1:1 and the rubber-band settle is the platform's own scroll physics. Every constant in the file carries a note saying what was measured to pick it, from the vertical falloff curve to the grain amplitude.
