...
Toasts stack at the bottom of the screen. Each new one springs up from below the edge; the ones already there step up 14px above the front toast's top edge and shrink 5% per slot, so toasts from one to three lines tall stack evenly. The front toast follows a downward drag and rubber-bands against an upward one. A drag past 56px, or a flick past 800 px/s, commits the dismiss with a light haptic, and the toasts behind spring forward. Every toast auto-dismisses after 3 seconds and carries an optional action button plus a close button.
...
- Measure toast heights and offset the stack from the front toast's top edge. Every toast is anchored to the same bottom edge, so a taller toast behind a shorter one would poke out by the height difference. `onLayout` reports each height to the provider, which passes back `height` and `frontHeight`; `stackOffset` lines each top edge up 14px per slot above the front. A new toast waits for both measurements before its first stack offset.
