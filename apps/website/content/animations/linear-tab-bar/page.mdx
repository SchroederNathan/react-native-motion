...
- The pill-to-menu morph squeezes first (scaleX dips around 15% progress), then the height grows into the staggered menu.
- Overdrag stretches the glass along the drag axis with a damped curve and squeezes it slightly across it.
- A radial SVG gradient sits under the finger and fades out on release.
- In search mode the pill's width goes to zero as the search bar grows, and a close button slides in.
- The header, divider, and seven menu rows stagger in at 40ms intervals, reversed on close.
