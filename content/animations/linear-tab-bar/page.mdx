...
A **glassmorphic tab bar** inspired by Linear's mobile app. A frosted-glass pill holds tab icons that can be tapped or swiped between. Dragging up morphs the pill into a full menu panel with staggered item reveals. A search button expands into a search bar while the pill collapses away. Every interaction features a liquid glass stretch effect — the pill deforms elastically when dragged past its bounds — and a radial glow that follows the touch point.

Key animation behaviors:
- **Pill-to-menu morph**: The pill scales through a squeeze (scaleX dip at 15%) then expands in height to reveal a staggered menu
- **Liquid glass stretch**: Overdrag creates a damped elastic deformation with directional stretch and cross-axis compression
- **Touch-tracking glow**: A radial SVG gradient follows the finger and fades out on release
- **Search transition**: The pill width animates to zero while the search bar expands, with the close button sliding in
- **Staggered menu items**: Header, divider, and 7 menu items animate in with 40ms stagger delays, reverse on close
...
- **Use `Gesture.Race()` for the search button's tap + pan.** Here you want tap OR pan — not both. The tap handles toggle, the pan handles the stretch effect.
...
- **The liquid glass stretch uses damped overflow, not raw delta.** `dampedX = sign * MAX_PULL * (1 - 1 / (|overflow| / MAX_PULL + 1))` gives a natural rubber-band feel that asymptotically approaches MAX_PULL.

- **The pill-to-menu morph uses a non-linear scaleX curve.** It dips to 0.7 at 15% progress, recovers to 0.85 at 50%, and reaches 1.0 at 85%. This creates the characteristic "squeeze then expand" feel.
...
- **The glow overlay uses a two-phase progress.** Phase 1 (0→1): glow appears at touch point. Phase 2 (1→2): glow fades out while scaling up 4x, creating a ripple effect on release.

- **Keyboard animation uses `react-native-keyboard-controller`.** The entire tab bar translates up with the keyboard via `useReanimatedKeyboardAnimation`, keeping it accessible during search input.
