export const meta = {
  ...
  description: 'The ChatGPT media picker, rebuilt off a frame-by-frame reading of the real thing. One glass panel grows out of the + button, morphs into a full-bleed photo grid, and then shrinks into the composer carrying the photo you picked — the same surface the whole way through.',
...
- **One surface, three shapes**: the circle around the + (34pt), a 280pt menu, and the full-bleed grid. A derived rect mixes between them off two shared values, `open` and `morph`.
- **Scaled content, not re-layout**: the menu and the grid are laid out at their natural size, anchored top-left, and scaled by the panel's width ratio through the morph.
- **Shared-element flights**: each picked photo leaves the exact cell rect FlashList reports, and lands on a composer slot that is still rising as the strip opens.
- **Real liquid glass**: `expo-glass-effect` on iOS 26, an `expo-blur` stand-in below it, and a flat measured color on Android, where a blur hosted over the keyboard has nothing to sample.
- **Keyboard stays up**: the sheet lives in an `OverKeyboardView`, and every rect derives from the keyboard-tracked composer bottom.
- **Camera rides the same panel**: the sheet morphs into a camera preview with the grid's exact footprint, and a capture leaves as the whole sheet, shrinking into its composer slot.
