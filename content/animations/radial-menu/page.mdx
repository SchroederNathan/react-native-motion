...
A **long-press radial menu** from the touch point: blur dims the screen, the pressed card clones into the overlay, and circular actions spring out. Buttons scale with proximity and shift aside from the finger. Release on a button to select (haptics). Fan angles adjust so actions stay clear of the screen edges.
